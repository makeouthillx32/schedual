export async function getDevicesUsedData(
  ...
  try {
    ...
    const totalSessions = deviceStats.reduce((sum: number, device: any) => sum + device.sessions_count, 0);
    
    // Map to the expected format
    const result = deviceStats.map((device: any) => ({
      name: capitalizeDeviceType(device.device_type),
      percentage: totalSessions > 0 ? device.sessions_count / totalSessions : 0,
      amount: device.sessions_count,
    }));
