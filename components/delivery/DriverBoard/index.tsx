...
import { TrashRun, FilterView } from "@/components/delivery/_components/types";
...
export default function DriverBoard({ supabase, isDark, onCountsChange }: DriverBoardProps) {
  ...
  const deleteOrder = async (order: DeliveryOrder) => {
    const ok = window.confirm(
      `Remove "${order.customer_name}" (${order.order_type})? This cannot be undone.`
    );
    if (!ok) return;
    const snapshot = [...orders];
